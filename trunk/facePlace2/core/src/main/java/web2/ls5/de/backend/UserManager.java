...
public class UserManager {
  ...
  public boolean signUp(String name,
    ...
    log.info("1. schritt");
    DBPerson xperson = new DBPerson(name, hashedPassword, birthdate, gender);
    pers = xperson;
    ...
    return true;
  	...
  	public boolean isValid(String username, String password) 
  	{
  		...
  		String saltedPassword = hashText + password;
  		String hashedPassword = generateHash(saltedPassword);
  		Query q = em.createQuery("select password from DBPerson s where s.name = " + username);
